...
function Options {
	...
	read -p"$GREEN[#NOTDEAD#]$WHITE-->" recipient
	while [ "$recipient" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
        read -p"$GREEN[#NOTDEAD#]$WHITE-->" recipient
	...
	read -sp"$GREEN[#NOTDEAD#]$WHITE-->" password
	while [ "$password" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
        read -sp"$GREEN[#NOTDEAD#]$WHITE-->" password
	...
	read -p"$GREEN[#NOTDEAD#]$WHITE-->" destination
	while [ "$destination" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
        read -p"$GREEN[#NOTDEAD#]$WHITE-->" destination
	...
	read -p"$GREEN[#NOTDEAD#]$WHITE-->" server
	while [ "$server" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
        read -p"$GREEN[#NOTDEAD#]$WHITE-->" server
	...
	read -p"$GREEN[#NOTDEAD#]$WHITE-->" port
	while [ "$port" = "" ];
		do
	...
	read -sp"$GREEN[#NOTDEAD#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#NOTDEAD#]$WHITE-->" up_pass
...
function installer {
	...
	if [ $confvar == 1 ]; 
		...
		cd ../
		cd ../
		echo "Path = `pwd`">>Devilx/Configuration/Configuration.ini
		sleep 2
		printf "${GREEN}\n\n[+]${WHITE}PROGRAM INSTALLED CORRECTLY${GREEN}[+]"
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED Devilx\n\n"
		exit 1
    fi
